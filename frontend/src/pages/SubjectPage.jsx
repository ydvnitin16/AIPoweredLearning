...
import { Plus, Settings, CheckCircle, Pencil, Trash2, Sparkles } from "lucide-react";
...
export default function SubjectPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      {/* Subject Header */}
      <header className={`relative p-10 text-white rounded-b-3xl shadow-lg bg-gradient-to-r ${subject.color}`}>
        <div className="flex justify-between items-start">
          <div>
            <h1 className="text-3xl font-bold">{subject.title}</h1>
            <p className="mt-2 text-sm opacity-90">Progress: {subject.progress}%</p>
          ...
          {topics.map((topic) => (
            ...
            >
              <h3 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
                {topic.done && <CheckCircle size={18} className="text-green-500" />}
                {topic.title}
              </h3>
              <p className="text-sm text-gray-500 mt-2">
                {topic.done ? "Completed" : "In Progress"}
              </p>
